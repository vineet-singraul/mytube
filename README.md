...
React + Node.js (Express) + MongoDB se bana ek "YouTube-jaisa" video app, jo **bina internet ke** (sirf local WiFi par) chalta hai. Koi login/signup nahi — sirf ek admin panel hai jahan se aap YouTube links add karte ho, aur wo videos automatically download ho kar users ke liye ready ho jaate hain.

## Ye kaam kaise karta hai

Do jagah, ek hi code chalega:

1. **Admin machine** (jahan internet hai) — Aap yahan admin panel se YouTube links (ek ya bahut saare) paste karte ho. Server yt-dlp se actual video file download karke apne paas store kar leta hai.
2. **Export** — Ek command chalane se naye download hue videos ek folder me copy ho jaate hain (USB me le jaane ke liye).
3. **Offline machine** (jahan network nahi hai) — Us USB folder ko "import" karte ho. Videos wahan ke storage + MongoDB me aa jaate hain.
...
- [MongoDB Community Server](https://www.mongodb.com/try/download/community) — **dono** machines par local install karna hai. Offline machine par internet nahi hoga isliye MongoDB Atlas (cloud) wahan kaam nahi karega — local MongoDB hi lagana.
...
> `npm install` ke time `yt-dlp-exec` package apne aap yt-dlp binary download karta hai — isliye pehli baar install karte waqt internet chahiye hoga.
...
npm install
copy .env.example .env
# .env me ADMIN_KEY waisi hi rakhein jaisi admin machine par thi (ya alag bhi rakh sakte hain)
```
...
4. Bas — YouTube jaisi grid dikhegi, video pe tap karke offline dekh sakte hain.
...
`.env` me `MAX_VIDEO_HEIGHT` se control hota hai (default `480`). Kam height = chhoti file size = kam storage aur kam data — low-connectivity/limited-storage deployment ke liye better.

## Project structure

```
server/            Express API + MongoDB + yt-dlp downloader
  src/
